...
const SinglePatient = ({ patient }: { patient: patient }) => {
    ...
    const patientAge = (patient: patient) => {
        return differenceInCalendarYears(new Date(), patient?.birthDate as Date);
    };
    return (
        <Link
            href={`/dashboard/patients/${patient.id}`}
            key={patient.id}
            className="flex justify-between items-center p-4 hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer transition-colors"
        >
            ...
            <div className="flex flex-col justify-start h-full">
                <p className="text-red-800 text-[12px] capitalize">{patient.medicalConditions}</p>
            </div>
        </Link>
    );
};
