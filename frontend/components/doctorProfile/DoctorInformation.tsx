...
const DoctorInformation = ({ id }: { id: string | undefined }) => {
    ...
    return (
        <section
            className={`flex flex-col bg-[var(--sidebar-background)] rounded-md p-4 lg:flex-row ${isFetching && 'min-h-[145px]'} `}
        >
            {isFetching ? (
                <Spinner
                    className="relative"
                ...
                <div className="flex justify-center items-center w-full h-full">
                    <p className="text-slate-600 text-sm font-medium capitalize">{error.message}</p>
                </div>
            ) : (
                <div className="flex justify-center items-center w-full h-full">
                    <p className="text-slate-600 text-sm font-medium capitalize">no user</p>
                </div>
            )}
        </section>
    );
