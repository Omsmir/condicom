            ...
            await toggleMulti.mutateAsync(formData, {
                onSuccess: async response => {
                    if (session?.user.mfa_state) {
                        ...
                        await new Promise(res => setTimeout(res, 1500));
                        router.push(`/dashboard/settings/setting/verify/${response.data.token}`);
                    }
                },
            });
        } catch (error: any) {
            ...
            >
                <div className="flex flex-col sm:flex-row items-center">
                    <div className="flex w-full flex-col space-y-2">
                        <div className="flex">
                            <h1 className="font-medium capitalize">Multi-factor Authentication</h1>
                        </div>
                        <p className="text-xs text-slate-500">
                            Enabling multi-factor authentication will add extra layer of security to
                            your department account
                        </p>
                    </div>
                    <div className="flex flex-col h-full w-full sm:w-fit space-y-2 mt-4 sm:mt-0">
                        <SubmitButton
                            className={clsx('bg-blue-800 text-slate-50 max-h-[25px] capitalize', {
                        ...
                        >
                            {session?.user.mfa_state ? 'disable multi-auth' : 'enable multi-auth'}
                        </SubmitButton>
                        ...
                        <div className="flex items-center text-xs capitalize whitespace-nowrap">
                            ...
                            <p
                                className={`ml-1 text-xs ${
                                    session?.user.mfa_state ? 'text-green-500' : 'text-red-800'
                                }`}
                            >
