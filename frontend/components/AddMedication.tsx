...
import { UseCreateMedication, UseCreateNotification } from '@/actions/mutation';

const AddMedication = () => {
    const [current, setCurrent] = useState(0);

    const { api, drawerState, setDrawerState, isLoading } = DashboardHook();

    const createMedication = UseCreateMedication(api);
    ...
    const NotificationToSwitch = (state: boolean, id?: string) => {
        return CreateNotification({
            ...
            description: state ? `Updated medication with ID:` : 'Created a new medication',
            ...
            createMedication.mutate(formData, {
                onSuccess: async (data, error) => {
                    ...
                    const notificationData = NotificationToSwitch(
                        data?.data.stateOfCreation,
                        data?.data.medication._id
                    );

                    createNotifcation.mutate(notificationData);
                },
            });
        } catch (error: any) {
