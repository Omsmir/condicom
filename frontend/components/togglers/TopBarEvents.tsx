        ...
        <React.Fragment>
            ...
            <DropdownMenuContent className="w-40 bg-slate-100 p-0 dark:bg-[var(--sidebar-accent)] dark:border-slate-800">
                ...
                <DropdownMenuCheckboxItem
                    className="hover:bg-slate-200 dark:hover:bg-[var(--sidebar-background)] cursor-pointer"
                    checked={WeekView}
                    onCheckedChange={() => handleView({ text: "Week'view", onclick: setWeekView })}
                ...
                <DropdownMenuCheckboxItem
                    className="hover:bg-slate-200 dark:hover:bg-[var(--sidebar-background)] cursor-pointer"
                    checked={DayView}
                    onCheckedChange={() => handleView({ text: "Day'view", onclick: setDayView })}
...
export const SwitchDay = () => {
    const { state, setState, calendarDays, viewPort, setCurrDate } = CalenderHook();
    ...
    const editedDays = Array.from(Days);
...
export const ExportAsCSV = ({ table }: { table: Table<any> }) => {
    const data = table.getFilteredSelectedRowModel().rows.map(row => {
        return [row.original.id, row.original.name, row.original.status, row.original.verified];
    });
...
export const exportToPDF = (table: Table<any>) => {
    ...
    const tableData = table.getFilteredSelectedRowModel().rows.map(row => {
        return [
            row.original._id,
    ...
    const tableHeaders = ['ID', 'Name', 'phone', 'country'];
...
export const CreateNotification = (props: NotifcationProps) => {
    const { type, title, description, assignedBy, user, eventId, eventType } = props;
...
export const NotificationAssignedBy = (notification: Notification, session: any) => {
    ...
    if (notification.user === session?.user.id) {
        ...
        return <p className="text-slate-500 font-medium mr-1">{notification.assignedBy} has</p>;
    }
};

export const NotificationDescription = ({ notification }: { notification: Notification }) => {
    const Component = () => {
