...
export const generateExpirationDate = ({ month }: { month: string }) => {
  const days = plans[month]; // Directly get the value from the object

  if (!days) throw new Error("Invalid plan"); // Handle invalid input

  const expirationDate = new Date();
  expirationDate.setDate(expirationDate.getDate() + days); // Simpler way to add days

  return expirationDate;
};

console.log(generateExpirationDate({ month: "month" })); // ✅ Works as expected
console.log(generateExpirationDate({ month: "1year" })); // ✅ Works
console.log(generateExpirationDate({ month: "invalid" })); // ❌ Throws error
