...
export const getCodes = async (query:FilterQuery<CodeDocument>,limit:number,cursor?:string) => {
  if (cursor) {
    query.createdAt = { $gt: new Date(cursor) };
  }

  const nextCursor = codes.length === limit ? codes[codes.length - 1].createdAt.toISOString() : null;

return const codes =await CodeModel.find(query).sort({ createdAt: -1 }).limit(limit)
}
