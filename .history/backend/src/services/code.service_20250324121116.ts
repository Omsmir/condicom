...
export const getCodes = async (
  ...
  const codes = await CodeModel.find(query).sort({ _id: -1 }).limit(limit + 1);

  let nextCursor: string | null = null;
  if (codes.length > limit) {
    nextCursor = codes[limit]._id as string; // Use the last item's _id as the nextCursor
    codes.pop(); // Remove the extra item from the results
  }

  console.log(codes.leng)
