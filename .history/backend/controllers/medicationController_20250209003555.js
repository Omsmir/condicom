...
export const createMedication = async (req, res, next) => {
  ...
  try {
    if (existedMedication) {
      ...
      return res.status(201).json({ medication:existedMedication,message: "Quantity added successfully" ,stateOfCreation:true});
    }
...
export const GetSpecificMedication = async (req,res,next) => {
  ...
  try {
    ...
    return res.status(200).json({medication:existingMedication,message:"success"})
  } catch (error) {
    next(error)
    return res.status(500).json({message:error.message})
  }
