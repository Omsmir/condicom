...
export const createMedication = async (req, res, next) => {
  ...
  const existedMedication = await Medication.findOne({
    name: medication.name,
    strength: medication.strength,
  });
  try {
...
export const GetSpecificMedication = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ medication: existingMedication, message: "success" });
  } catch (error) {
    next(error);
    return res.status(500).json({ message: error.message });
  }
...
export const DeleteMedication = async (req, res, next) => {
  ...
  try {
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res.status(400).json({message:"Medication Id Is Not a valid MongooseId"})
    }
    const MedicationExisted = await Medication.findByIdAndDelete({_id:id})

    if(!MedicationExisted){
      return res.status(404).json({message:"medication doesn't exists"})
    }

    await MedicationExisted.deleteOne()

    return res.status(201).json({message:"medication deleted successfully"})
  } catch (error) {
    next(error)
    return res.status(500).json({message:error.message})
  }
