...
export const refreshAccessToken = async (token: any) => {
  try {
    ...
    const decodedToken = jwt.decode(newAccessToken) as Jwt;

    return {
      ...token,
      accessToken: newAccessToken,
      expiresAt: decodedToken?.exp ? decodedToken.exp * 1000 : token.expiresAt,
      id: decodedToken?.id || token.id,
      role: decodedToken?.role || token.role,
      name: decodedToken?.name || token.name,
      profileImg: decodedToken?.profileImg || token.profileImg,
