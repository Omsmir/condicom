...
const AddMedication = () => {
  ...
  const createMedication = UseCreateMedications()
  ...
  const onSubmit = async (values: Zod.infer<typeof MedicationSchema>) => {
      ...
      switch (state) {
        ...
        case false:
          return (data = CreateNotification({
            ...
            description: "updated medication with id",
            assignedBy: `${userRole(session)} ${session?.user.name}`,
            user: `${session?.user.id}`,
            eventType: "updating",
            eventId:id
    ...
    try {
      ...
      const newMedication = (await response.data) 

  
      if (newMedication.stateOfCreation) {
        const notificationData = NotificationToSwitch(false,newMedication.medication._id);
