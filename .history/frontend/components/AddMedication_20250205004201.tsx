...
import { Medications } from "@/lib/vaildation";
...
const AddPatient = () => {
  ...
  const form = useForm<Zod.infer<typeof Medications>>({
    resolver: zodResolver(Medications),
    defaultValues: {
      firstName: "",
      lastName: "",
      email: "",
  ...
  const onSubmit = async (values: Zod.infer<typeof Medications>) => {
    setIsLoading(true);
