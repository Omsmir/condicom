...
import { Notification, Notifications, ObjectType } from "@/types";
import {
  CheckCircleFilled,
  DeleteFilled,
...
export const playSound =  (tone: keyof ObjectType) => {
...
export const SocketInitiator = async (
  ...
  const handleNewNotification = async (data:Notifications) => {
    await queryClient.setQueryData(["userNotifications",session?.user.id],data)

     playSound(tone);
  };

  socket.on(notificationTO, async (data: Notifications) => {
    try {
