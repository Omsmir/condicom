...
export const DeleteHandler = ({
  ...
  const {data,mutateAsync,isSuccess,isError} = UseDeleteMedication(api);

  console.log(isSuccess)
  const HandleDeletion = async () => {
    try {
      const response = await mutateAsync(id).then((data) => {
        return api.success({
          message: data?.d || "Deleted successfully!",
    ...
    try {
      Swal.fire({
        ...
        if (result.isDenied) {
          ...
          const deletionResponse = await HandleDeletion();
