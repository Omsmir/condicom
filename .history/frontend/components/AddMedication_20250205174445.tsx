...
const AddMedication = () => {
  ...
  const onSubmit = async (values: Zod.infer<typeof MedicationSchema>) => {
    ...
    const NotificationToSwitch = (state: boolean) => {
      let data: any;
      switch (state) {
        case true:
          return (data = CreateNotification({
            type: "medication creation",
            title: "Medication Creation",
            description: "created a new medication",
    ...
    console.log(NotificationToSwitch(false))
    const data = {
      name: values.name,
    ...
    try {
      const response = await axios.post(
        "http://localhost:8080/api/medications/create",
        formData
      );
      ...
      const newMedication = (await response.data.medication) as
        | medication
        | undefined;

      const existingMedication = await getSpecficMedication(newMedication?._id);
      console.log(newMedication?._id ,existingMedication)

      if (existingMedication) {
        const notificationData = NotificationToSwitch(false);
        ...
        const notificationData = NotificationToSwitch(true);
