  ...
  return (
    <React.Fragment>
      {DaysToMapThrough().map((day, index) => (
        <div className="grid border-r pt-2 dark:border-slate-700 " key={index}>
          {theMinutes.map((hour, hourIndex) => {
            ...
            return (
              <div
                className="flex justify-center cursor-pointer border-b h-20 relative dark:border-slate-700 hover:bg-[var(--sidebar-background)]  dark:hover:bg-[var(--sidebar-accent)]"
                key={hourIndex}
              >
                {filteredTasks &&
