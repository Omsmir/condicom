      ...
      {DaysToMapThrough().map((day, index) => (
        <div className="grid border-r py-4 dark:border-slate-700" key={index}>
          {theMinutes.map((hour, hourIndex) => {
            ...
            return (
              <div
                className="flex justify-center cursor-pointer border-b h-20 relative dark:border-slate-700 bg-[var(--sidebar-background)] dark:hover:bg-[var(--sidebar-background)]"
                key={hourIndex}
              >
                {filteredTasks &&
