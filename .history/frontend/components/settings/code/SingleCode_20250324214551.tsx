...
const SingleCode = ({
  ...
  const data = {
    ...
    role: code.role,
    "expiration date": format(code.expiration, "p"),
    "created at": format(code.expiration, "p"),
  };
  return (
    <div className="grid grid-cols-12 col-span-6 sm:col-span-4 md:col-span-3 mr-2 last-of-type:mr-0 p-4  bg-[var(--sidebar-background)] rounded-md shadow-md shadow-slate-300 dark:shadow-slate-800">
      {Object.entries(data).map(([key, value]) => (
        ...
        >
          {" "}
          <CustomSkeleton SkeletonType={SkeletonType.HEAD} innerText={value} loading={isLoading} classname="min-w-24"  />
        </SingleInformationRow>
      ))}
    </div>
  );
