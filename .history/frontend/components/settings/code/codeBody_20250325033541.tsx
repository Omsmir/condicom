...
const CodeBody = ({ codes, users, role }: CodeBodyProps) => {
    const appliedFilter = (codes:code[],filter:keyof code ,filterValue:string | undefined) => {
       return codes.filter((code) => { if(filterValue === "clear" || filterValue === "" ){
    ...
    <div className="grid grid-cols-12 w-full mt-4 ">
      {codes.isFetching ? (
        ...
        codes.data.pages.map((ele) =>(
            appliedFilter(ele.codes,"role",role).map((code) => (
                <SingleCode
                code={code}
                key={code._id}
                users={users}
