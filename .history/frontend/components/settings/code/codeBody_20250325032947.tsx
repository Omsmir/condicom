...
import { code, ObjectType, UserInformation } from "@/types";
...
const CodeBody = ({ codes, users, role }: CodeBodyProps) => {
    const appliedFilter = (codes:code[],filter:keyof ObjectType ,filterValue:string) => {
        codes.filter((code) => code[`${filter}`])
    }
  return (
