...
import { UseDeleteCode } from "@/actions/mutation";
...
const DeleteCode = ({code}:codeDeletionInterface) => {
  const { isDeleteLoading ,open,setOpen} = AccountHook();
  const {contextHolder,api} = DashboardHook()

  const {data:session} = useSession()
  const deleteCode = UseDeleteCode(api,session?.user.id)
  ...
  return (
    <CustomDialog open={open} setOpen={setOpen} DialogTitle="delete code" DialogType={DialogTypes.CONFIRM}>
        {contextHolder}
      <Form {...form}>
        <form
