...
const CodeBody = ({ codes, users, role, used }: CodeBodyProps) => {
  ...
  const appliedFilter = (
    ...
    return codes.filter((code) => {
      switch (filter) {
        case "role":
          if (filterValue === "clear" || filterValue === "") return code;
          return code[`${filter}`] === filterValue;
        case "used":
            if (filterValue === "clear" || filterValue === undefined) return code;
            return code[`${filter}`] === false
      }
    });
  };
  return (
      ...
      {codes.isFetching ? (
        ...
        codes.data.pages.map((ele) =>
          Object.entries(Filters).map(([key, value]) => {
            return appliedFilter(ele.codes, key as keyof code, value).map(
              (code) => (
                <SingleCode
                  code={code}
                  key={code._id}
