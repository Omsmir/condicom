...
const Preferences = ({ code }: { code: code }) => {
  ...
  return (
    ...
    {isAfter(newExpirationDate, new Date()) && <ShadBadge className="bg-slate-800 text-slate-50  rounded-md mr-1">new</ShadBadge>}
    <div className="flex">
     {code.used ? <Badge status="success" /> : <Badge status="error" />}
     </div>
    </div>
  );
};
const SingleCode = ({ code, users, isLoading }: SingleCodeProps) => {
  ...
  const iterateValueStyle = (value:number) => {
    const parseIntV = parseInt(value)
