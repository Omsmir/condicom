    ...
    case true:
      return (
        <Fragment key={sideItemTitle}>
          <SidebarGroupLabel className={clsx("flex text-slate-500 capitalize w-full whitespace-nowrap",{"hidden":!open})}>
            {sideItemTitle}
          </SidebarGroupLabel>
          {children}
        </Fragment>
        ...
        {items.map((item) => {
          if (item.group) {
            if (item.private) {
              ...
              return (
                <CheckSidebarTitleExistance  key={item.title} sideItemTitle={item.sideItemTitle} sideItemState={item.sideItemState}>
