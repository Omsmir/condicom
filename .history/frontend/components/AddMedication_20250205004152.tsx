...
import { Pharmc } from "@/lib/vaildation";
...
const AddPatient = () => {
  ...
  const form = useForm<Zod.infer<typeof Pharmc>>({
    resolver: zodResolver(Pharmc),
    defaultValues: {
      firstName: "",
      lastName: "",
      email: "",
  ...
  const onSubmit = async (values: Zod.infer<typeof Pharmc>) => {
    setIsLoading(true);
