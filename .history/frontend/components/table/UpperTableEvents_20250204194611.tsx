...
interface UpperTableEventsProps<TData,TValue> {
  ...
  columnFilters?: ColumnFiltersState; | un
...
const UpperTableEvents = <TData,TValue> ({
  ...
  return (
    <div className="flex justify-between items-center p-2">
      <div className="flex mr-2 sm:m-0">
        <Input
          placeholder="Search by name"
          value={globalFilter}
          onChange={(e) => setGlobalFilter(e.target.value)}
          className=" focus:outline-0  shad-input max-h-[30px]"
        />
        <ColumnFilters columns={columns} setColumnFilters={setColumnFilters} columnFilters={columnFilters} />
      </div>
