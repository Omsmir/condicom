...
const ColumnFilters = <TData, TValue>({
  ...
  const setColumnFilterForRows = ({e, columnId,switchValue}:{e?:any,columnId:string,switchValue:string}) => {
    ...
    switch(switchValue){
        case "gender":
            return setColumnFilters((prev) =>
              ...
              case "input":
                return setColumnFilters((prev) => [...prev.filter((ele) => ele.id !== columnId),{id:columnId,value:e.target.value}])
              default: return null
    }
  };
                ...
                return (
                    columnId === "gender" && (
                      <Select
                        key={value}
                        onValueChange={(e) => {
                          setColumnFilterForRows({e,columnId,switchValue:"gender"})
                        }}
                  ...
                  case "firstName":
                    return (
                        <Input key={value} value={value ?? undefined} onChange={(e) => {setColumnFilters((prev) => [...prev.filter((ele) => ele.id !== columnId),{id:columnId,value:e.}])}} />
                    )
                  default: return null
          }
      })}
