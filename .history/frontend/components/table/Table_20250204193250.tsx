          ...
          return columnId === "gender" && (
            <Select   onValueChange={(e) => {
              setColumnFilters((prev) => 
                e ? [...prev.filter((f) => f.id !== columnId), { id: columnId, value: e }] 
                  : prev.filter((f) => f.id !== columnId) // Remove filter if empty
              );
            }} value={value || ""}>
              <SelectTrigger className="shad-select-trigger text-dark-500 max-w-[145px]">
                <SelectValue placeholder={"gender filter"} />
              </SelectTrigger>
            <SelectContent className={cn("shad-select-content max-h-[250px]")}>
