...
export const pharmacyHiddenColumns: ColumnDef<medication>[] = [
  {
    accessorKey: "supplier",
    header: () => <p className="text-slate-500 uppercase"> supplier</p>,
    cell: ({ row }) => {
      const supplier = row.original.supplier || "N/A";
...
export const pharmacyColumns: ColumnDef<medication>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (quantity >= 50) {
        return <p className="text-green-700 ">{quantity}</p>;
      } else {
        return <p className="text-red-700 ">{quantity}</p>;
      }
    },
  },
  {
  ...
  {
    id: "expander",
    header: () => null, // No header for the expand button
    cell: ({ row }) => (
      <Button
        onClick={() => row.toggleExpanded()}
        className="p-2 rounded text-12"
      >
        {row.getIsExpanded() ? <ArrowUpOutlined /> : <ArrowRightOutlined />}
      </Button>
    ),
  },
