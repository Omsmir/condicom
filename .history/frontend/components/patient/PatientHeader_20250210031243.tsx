...
const PatientHeader = ({ patient,id }: { patient: patient | undefined ,id:string | undefined}) => {
  const { loading, setLoading } = PatientHook();

  const Patient = useSpecificPatient(id)

  console.log(Patient.data.pa)
          ...
          <div className="flex items-center h-full">
            ...
            <div className="flex flex-col items-start justify-start ml-2">
              <CustomSkeleton
                ...
                loading={loading}
                classname="w-32 min-h-[25px] font-medium capitalize"
                innerText={`${patient?.firstName} ${patient?.lastName}`}
              />
              <CustomSkeleton
                SkeletonType={SkeletonType.HEAD}
                loading={loading}
                classname="w-28 text-slate-600 capitalize text-sm"
