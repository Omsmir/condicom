...
export const MedicationSchema = z.object({
  name:z.string({message:"please write the medication name"}).min(4,"min characters is 4").max(30,"max characters is 30"),
  generic_name:z.string({message:"please select the generic name"}),
  description:z.string().optional(),
  form:z.enum(["tablet"],{message:"please select the medication form"}),
  strength:z.enum(["500mg"],{message:"please select a medication strength"}),
  Route:z.enum(["oral"],{message:"please select a medication route"}),
  manufacturer:z.string().min(5).max(25).optional(),
  batch_number: z.string().min(5).max(25).optional(),
  storage_conditions: z.string().min(5).max(45).optional(),
  expiryDate:z.date({message:"please select expiry date"}).refine((date) => date > new Date() ,{message: "Date must be in future"}),
  drug_category:z.enum(["analgesic"],{message:"please select medication category"}),
  price:z.number({message:"please write down a price"}).min(1,{message:"min price is 1"}).max(9999,{message:"max price is 9999"}),
  stock_Quantity:z.number({message:"please write down a stock quantity"}).min(1,{message:"min quantity is 1"}).max(9999,{message:"max price is 9999"})
})