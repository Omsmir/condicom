...
import {  characters, codeExpirationPlans, firstNumber, FiveNumber, genders, medicationCategories, medicationForms, medicationRoutes, medicationStrengths } from "./constants";
...
export const patientSchema = z.object({
  ...
  currentMedications: z.string({message:"please describe your medications"}).min(1,"please describe your medications"),
...
export const MedicationSchema = z.object({
  ...
  stock_quantity:z.coerce.number({message:"please write down a stock quantity"}).min(1,{message:"min quantity is 1"}).max(9999,{message:"max quantity is 9999"})
})
...
export const CodeDeletionSchema = z.object({
  code:z.string({message:"please write down the code"}).refine((code) => code.match(/B(1|2)[0-5](C|D|E)[0-9]{5}/g),{message:"invalid code"})
})