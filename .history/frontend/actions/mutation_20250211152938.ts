...
import { LoginApi, LoginApiProps } from "./User";
...
export const useLogin = (api:NotificationInstance) => {
  return useMutation({
    mutationFn: async ({email,password}:LoginApiProps) => LoginApi({email,password}),
    onError: (error) => {
      HandleAxiosErrors({api:api,error:error})
