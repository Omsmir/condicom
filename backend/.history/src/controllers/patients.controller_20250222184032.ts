...
import { DeletepatientSchemaInterface, GetpatientSchemaInterface, patientSchemaInterface } from "../schemas/patient.schema";
...
export const DeletePatientHandler = async (req: Request<DeletepatientSchemaInterface['params']>, res: Response) =>{
    try {
        const id = req.params.id
        const Patient = await getPatient({_id:id})
        ...
        await deleteImage(Patient.profileImg?.p)
