...
import { GetnotificationSchemaInterface, notificationSchemaInterface } from "../schemas/notifications.schema";
...
export const createNotificationHandler = async (
  ...
  try {
    ...
    if (currentUser.role === "Admin") {
      ...
      res.status(201).json({message:"notification created successfully",notifications: notification,})
  } catch (error:any) {
...
export const getUserNotificationsHandler = async (req:Request<GetnotificationSchemaInterface['params']>,res:Response) => {
    try {
        const id = req.params.id

        const currentUser = await fin
    } catch (error) {
